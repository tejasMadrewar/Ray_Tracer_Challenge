...
void mat::from_vec(const std::vector<float> &ar) {
  if (ar.size() < (Row * Col))
    throw;
  for (int r = 0; r < Row; r++) {
    for (int c = 0; c < Col; c++) {
...
bool mat::operator==(const mat &Mat) {
  // return (Row == Mat.Row && Col == Mat.Col && values == Mat.values);
  const float EPSILON = 0.0001;
  if (Row == Mat.Row && Col == Mat.Col && values.size() == Mat.values.size()) {
...
mat mat::transpose() { // make transpose matrix of given size
  mat result(Col, Row);
...
float mat::cofactor(int r1, int c1) {
  if ((r1 + c1) % 2 == 0)
...
mat mat::Identity(int size) { // make Identity matrix of given size
  mat result(size);
  result.Col = size;
  result.Row = size;
